var str = '{{auth_url}}',
  ...
  function addKey(call) {
    ...
    if (call == 'auth') {
    el.setAttribute("href", "{{auth_url}}?response_type=code&client_id=" + clientId + "&client_secret=" + clientSecret + "&redirect_uri=" + "{{provider.domain}}" + "/docs&state=3scale");
    } else if (call == 'token') {
  ...
  {% assign code = params | split: 'code=' | last | split: '&' | first %}
    ...
    var  formData = "response_type=code&client_id=" + clientId + "&client_secret=" + clientSecret + "&redirect_uri=" + "{{provider.domain}}" + "/docs&state=3scale&grant_type=authorization_code&code=" + code;  //Name value Pair
